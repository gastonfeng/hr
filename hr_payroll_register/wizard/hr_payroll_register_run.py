...
class payroll_register_run(orm.TransientModel):
    ...
    def create_payslip_runs(self,  ids, context=None):
        ...
        pr = reg_pool.browse( register_id, context=context)
        ...
        for dept in dept_pool.browse(
             data['department_ids'], context=context
        ):
            run_res = {
            ...
            run_id = run_pool.create( run_res, context=context)

            slip_ids = []
            ee_ids = ee_pool.search(
                 [('department_id', '=', dept.id)],
                order="name", context=context)
            for ee in ee_pool.browse( ee_ids, context=context):
                ...
                slip_ids.append(
                    slip_pool.create( res, context=context))
            slip_pool.compute_sheet( slip_ids, context=context)
